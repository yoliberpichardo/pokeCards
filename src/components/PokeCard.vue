...
export default {
        ...
        changeStar(id){
            this.pokeData.forEach(element => {
                if(element.id === id){
                    ...
                    localStorage.setItem('pokeFavorite',JSON.stringify(this.use.pokemonFavorite))
                } 
            })
        },
        changeInfo(id){
    ...
    mounted(){
        if(localStorage.getItem('pokeFavorite')) {
            try {
                this.use.pokemonFavorite = JSON.parse(localStorage.getItem('pokeFavorite'));
            } catch(e) {
                localStorage.removeItem('pokeFavorite');
            }
        }
    },
}
